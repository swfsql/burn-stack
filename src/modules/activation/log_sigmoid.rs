...
//! [`softplus`](crate::modules::softplus) already branches to the identity
//! past a per-dtype threshold, and this keeps the large-negative tail
//! (`log σ(x) → x`) finite. A direct `log(1 / (1 + e^−x))` overflows `e^−x`
//! there, and collapses to `−∞` with a `NaN` gradient.
